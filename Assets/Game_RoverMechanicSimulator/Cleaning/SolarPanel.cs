    ...
    private void Update() {
        if (Input.GetMouseButton(0)) {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out RaycastHit raycastHit)) {
                ...
                //brush scaler
                int brushWidth = Mathf.RoundToInt(dirtBrush.width * brushScale);
                int brushHeight = Mathf.RoundToInt(dirtBrush.height * brushScale);

                int pixelXOffset = pixelX - (brushWidth / 2);
                int pixelYOffset = pixelY - (brushHeight / 2);

                for (int x = 0; x < brushWidth; x++)
                {
                    for (int y = 0; y < brushHeight; y++)
                    {
                        int dirtBrushX = Mathf.FloorToInt((float)x / brushScale);
                        int dirtBrushY = Mathf.FloorToInt((float)y / brushScale);

                        if (dirtBrushX >= dirtBrush.width || dirtBrushY >= dirtBrush.height) continue;

                        Color pixelDirt = dirtBrush.GetPixel(dirtBrushX, dirtBrushY);
                        Color pixelDirtMask = dirtMaskTexture.GetPixel(pixelXOffset + x, pixelYOffset + y);
